...
<%--
# 요청값으로 DB 객체 생성
1. 요청값으로 호출된 내용을 여러개의 데이터로 VO 객체를 생성하는데 까지 처리하여, 다른 모듈에 매개값으로
처리되는 경우가 많다

2. 이 내용에서 처리한 중점사항
	1) 기본 요청화면 구현
	2) 요청화면에서 form 객체에 의해 넘겨지는 데이터가 어떤 유형인지 파악
		주의사항
		- number 형변환
		- date(DB) : 문자열로 받아서 db sql에서
			to_date(문자열, '형식')
			ex) sql : to date(? ,'YYYY-MM-DD')
				pstmt.setString(5, vo.setString
	...
	8) DAO 연동
	9) 모든 처리가 끝난후, js로 등록성공/실패 등에 대한 메시지 처리
 --%>
...
<body>
    <div class="container mt-3">
       ...
       <form onsubmit = "return checkValid()" method="post">
            ...
            <%--
            # 패스워드/패스워드 확인 등록시 반드시 check해야할 내용
             --%>
            <div class="mb-3 mt-3">
            ...
            <input type="text" class="form-control"  
              id="point" placeholder="초기포인트 입력" name="point">
            </div>
