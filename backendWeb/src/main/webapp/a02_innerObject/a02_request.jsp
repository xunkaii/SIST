...
1. jsp에서 client단에 있는 정보를 서버단에 전송해서,
	...
	- request.gestSession() session 객체 생성
2. 기능내용
	1) 요청값처리(*)
	2) 쿠키정보 전달
	3) request 범위 데이터 저장
...
5. 다양한 query string 처리 예시
	...
	2) back단(jsp)
		...
		==> backend(서버단호출)
		# 결국은 query string 요청값을 client(브라우저)에서 호출하는 것으로 서버단에서 이러한 요청값을
		전달하여 요청객체(request)로 처리하는 것이다
...
6. 단일 데이터 전송과 다중 데이터 전송
	...
	2) form 요소객체
		...
		<input type = "checkbox" name="ck01"	check가 된 다중
...
# query string에 의해서 데이터 받기
a02_request.jsp?name = 홍길동

ex) age, loc로 나이와 사는 곳을 데이터 입력하고
화면에 나이와 사는곳의 정보를 출력
페이지? key=value&key2=value2&key3=value3 
