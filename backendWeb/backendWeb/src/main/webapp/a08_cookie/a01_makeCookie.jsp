...
<%--
#Cookie란?
1. 웹 프로그래밍에 있어서 서버와 클라이언트 간에 접속하여 처리된 정보를 
	임시 저장하는 경우가 간간히 발생한다.
	이 때, 정보를 서버에 보관하여 사용하는 것을 session이라고 하고, 서버에 보관되는 
	메모리 부담으로 클라이언트에 저장하는 것을 쿠키라고 한다.

2. 쿠키 생성 시점
	1) 쿠키는 클라리언트에서 요청에 의해 서버 안에서 객체를 생성하여 다시 reponse 객체에 
	쿠키를 추가하여 클라이언트에 보내면서 클라이언트(브라우저)에 지정한 경로에 저장한다.
	
3. 쿠키 활용
	1) 쿠키가 서버에서 생성되어 클라이언트에 저장되면 다시 요청 객체
	request에 의해 서버에 보내지면 쿠키값을 key/value형식으로 호출하여
	사용할 수 있다.
	2) 쿠키는 동일한 키로 addCookie로 할당하며 동일한 키로 설정된
	내용을 변경해준다.
	price ==> 3000
	price ==> 5000 최종으로 설정된 값이 해당 쿠키의 key에
	할당된 값이 된다.
    	...
    	<h2></h2>
	  	<nav class="navbar navbar-expand-sm bg-dark navbar-dark">
	  		<div class="container-fluid">    	
				...
				style="width:200px;">쿠키 확인하기</button>
	     	</form>
	 	    </div>
	 	</nav>
