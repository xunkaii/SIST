...
<%--
# jstl 객체 선언과 사용
1. jstl에서는 기본적으로 mvc에서 넘겨온 vo객체를 el 태그형태로 사용하는 것이 일반적이다
	1) controller단 (request 범위로 설정)
		request.setAttribute("p01", dao.getPerson());
	2) view단 (session 범위로 변수 설정)
		<c:set var = "person" value="${p01}" scope = "session"/>
		${p01.name} property로 호출 처리 (request 범위)
		${person.name} property로 호출 처리 (session 범위)
	3) 데이터 변경 처리
		<c:set target = "person" property = "age" value = "30"/>
		${person.age}
2. 하지만 script를 통해서 객체를 선언하고 property를 통해 데이터를 핟당하여 처리하는 방법도 사용된다
--%>
<%
// controll단 설정 가정
...
<body>
    <div class="container mt-3">
    	...
    	<%--
    	2. request 범위 Person 객체를 session으로 이름을 변경해서 할당
    		기본에 request 범위의 p01개체는 session 범위로 변경이 된다
    	 --%>
    	...
    	<h3>session 범위 age:${p01.age}</h3>
    	<%--
    	ex) a04_object.jsp
    	Product를 클래스 prod로 request로 설정하고, session 범위로 proSess로 처리하여 호출하세요
    	물건 가격 변경 처리
    	--%>
<%
